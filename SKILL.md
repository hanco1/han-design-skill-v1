---
name: han-design-skill-v1
description: Generate Han-style editorial social card image sets and WeChat official account cover pairs from articles, scripts, screenshots, product notes, subtitles, or photos. Use when the user asks for 小红书图文, Rednote/Xiaohongshu images, social cards, carousel images, 3:4 covers, 微信公众号封面, WeChat 21:9 + 1:1 covers, Swiss Style, magazine-style social images, or explicitly asks to call this skill for editorial frontend/UI design using Han's warm-beige serif house style.
---
...
- Social images that need Han-style, Guizang-inspired Swiss or editorial magazine layouts.
...
## User Editorial House Style

When the user says to call/use this skill for frontend design, editorial style, high-end editorial feeling, or "my design理念", use this house style by default unless they specify another brand system.

Design direction:

- Warm beige base: use paper-like warmth to remove screen coldness. Prefer `#EFEAE0` page backgrounds and `#E5DFD2` light cards. Do not use pure white.
- Serif as protagonist: use bold serif/Songti as the main authority layer. Prefer Source Han Serif / Georgia for titles and body; do not make big headlines sans-serif.
- Keyword color highlight: recolor only 1-2 important words per screen/card, usually with `#CD6F47` or `#6B8A6F`.
- Large whitespace: one screen/card carries one viewpoint. Keep more than 20% visible breathing room and avoid overlapping ideas.
- Rounded, not sharp: use 14-16px radii for cards, image wells, and emphasis blocks. Keep it soft but not cartoonish.
- Mono micro labels: use JetBrains Mono only for overline labels, guides, metadata, and small navigation cues. Set uppercase with about `1.5px` letter spacing.
- Black emphasis blocks: use `#1A1A1A` as the emphasis-card color. Never use `#000000`.
...
- One screen/card = one core viewpoint.
- Highlight 1-2 keywords only.
- Radius is 14-16px.
- Whitespace is greater than 20%.
- Use `#1A1A1A`, not `#000000`.

Do not:

- Stack too many ideas on one screen/card.
- Use pure white backgrounds.
- Use sans-serif for large titles.
- Add shadows, gradients, glass, or fake 3D depth.
- Bold or color every word.
...
- `references/style-system.md` for Han editorial and Swiss visual rules adapted from the Guizang style language.
...
- Editorial Magazine x E-ink uses one of 7 magazine palettes: User Editorial Warm Beige (default when the user asks for their editorial house style), Ink Classic, Indigo Porcelain, Forest Ink, Kraft Paper, Dune, or Midnight Ink (the only dark variant; reserved for game key art / night photography / cinematic covers).
...
- Editorial: `<html data-theme="user-editorial | ink-classic | indigo-porcelain | forest-ink | kraft-paper | dune | midnight-ink">`.
...
- Never edit the original Guizang PPT skill, original social-card skill, or any upstream skill copied from elsewhere.
...
- In User Editorial House Style, do not use pure white, `#000000`, sans-serif display titles, gradient/shadow depth effects, or more than two highlighted keywords on one screen/card.
